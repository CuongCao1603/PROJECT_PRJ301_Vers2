...
public class LoadMoreControl extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        // Tạm thời load ra 3 sản phẩm trước
        // In ra cấu trức của từng sản phẩm
        
        String amount = request.getParameter("exits");
        int iamount = Integer.parseInt(amount);
        DAO dao = new DAO();
        //Dem duoc so luong san pham co tren trang jsp
        List<Product> listP = dao.getNext3Product(iamount);
        PrintWriter out = response.getWriter();

        for (Product o : listP) {
            // phải thêm class product khi đếm sản phẩm ở Home.jsp
